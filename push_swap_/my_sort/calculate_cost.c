...
int	eval_scenario(t_cost *best, t_scenario cur_scenar)
{
	...
	cur_best_scenar = get_max_value(best->moves_to_top_b, best->moves_to_top_a);
	new_potential_best = get_max_value(cur_scenar.tmp_moves_to_top_b,
...
void	calculate_cost(t_stack *a, t_stack *b, t_calc *current_elem,
	t_cost *best)
{
	t_scenario	cur_scenar;

	current_elem->pos_b = get_position(b, current_elem->value);
	current_elem->pos_a = find_position(a, current_elem->value);
	current_elem->sz_b = stack_size(b);
	current_elem->sz_a = stack_size(a);
	init_best_rr(best, current_elem->pos_b, current_elem->pos_a);
